...
import { scheduleConsultationContent } from "@/content/pages/schedule-consultation";
...
export default function ScheduleConsultationClient() {
  const tu = useTranslations("scheduleUI");
  const locale = useLocale() as "en" | "es";
  const content = scheduleConsultationContent[locale];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSubmitError(t(`errors.${data.errorKey || "generic"}`));
        return;
      }
      setSubmitted(true);
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    { name: tc("home"), href: "/" },
    { name: content.hero.eyebrow, href: "/schedule-consultation" },
  ]);
        ...
        <div className="site-container relative z-10">
          ...
          <div className="grid lg:grid-cols-[1fr_1fr] gap-12 lg:gap-16 items-start mt-12">
            ...
            >
              ...
              >
                {tu('startYourConsultation')} <ArrowRight size={16} />
              </motion.a>
            </motion.div>
              ...
              <div className="bg-white/[0.08] border border-white/10 backdrop-blur-sm rounded-lg p-8">
                {submitted ? (
                  ...
                  <form
                    ...
                    className="space-y-5"
                  >
                    <h2 className="text-xl font-semibold text-white mb-6">{content.form.heading}</h2>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        />
                        {errors.firstName && <p id="consult-firstName-error" role="alert" className="text-red-500 text-[13px] mt-1.5">{t(`errors.${errors.firstName}`)}</p>}
                      </div>
                      <div>
                        ...
                        />
                        {errors.lastName && <p id="consult-lastName-error" role="alert" className="text-red-500 text-[13px] mt-1.5">{t(`errors.${errors.lastName}`)}</p>}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {errors.email && <p id="consult-email-error" role="alert" className="text-red-500 text-[13px] mt-1.5">{t(`errors.${errors.email}`)}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {errors.phone && <p id="consult-phone-error" role="alert" className="text-red-500 text-[13px] mt-1.5">{t(`errors.${errors.phone}`)}</p>}
                    </div>
                    </div>
                    ...
                    <div>
                      ...
                      >
                        <option value="" className="bg-[#1B1E49]">{t('phChoosePractice')}</option>
                        {PRACTICE_OPTIONS.map((opt) => (
                          <option key={opt} value={tArea(`${opt}.label`)} className="bg-[#1B1E49]">{tArea(`${opt}.label`)}</option>
                        ))}
                      </select>
                      {errors.practice && <p id="consult-practice-error" role="alert" className="text-red-500 text-[13px] mt-1.5">{t(`errors.${errors.practice}`)}</p>}
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder={t('phMessageShort')}
                      />
                      {errors.message && <p id="consult-message-error" role="alert" className="text-red-500 text-[13px] mt-1.5">{t(`errors.${errors.message}`)}</p>}
                    </div>
                    ...
                    >
                      {loading ? t('sending') : content.form.submitLabel}
                    </button>
      ...
      <section className="py-12 bg-[#F8F6F2]">
        <div className="site-container">
          <div className="flex flex-wrap justify-center gap-8 text-[#1B1E49] text-[16px] font-semibold">
            ...
            <span className="text-gray-400">·</span>
            <span>{tu('supremeCourt')}</span>
            <span className="text-gray-400">·</span>
            <span>50+ Countries</span>
            <span className="text-gray-400">·</span>
            <span>{tu('seHablaEspanol')}</span>
          </div>
        </div>
      </section>
